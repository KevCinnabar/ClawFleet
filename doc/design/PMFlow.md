...
步骤 2: PM 分析需求
    ↓
步骤 3: PM 识别不清楚的细节，向用户提问
    ↓
步骤 4: 用户澄清需求
    ↓
步骤 5: PM 生成 PRD 并保存到文档平台
    ↓
步骤 6: 提交用户确认
    ↓
步骤 7: 用户审批 PRD
    ...
    ├─→ [审批通过分支]
    │   ↓
    │   步骤 8a: PM 根据 PRD 创建任务
    │   ↓
    │   步骤 9a: 任务拆解为 Epic/Feature 级别
    │   ↓
    │   步骤 10a: 锁定本迭代范围
    │   ↓
    │   步骤 11a: 通知开发团队开始工作
...
#### 分支 A：审批通过路径
- **触发条件**：用户确认 PRD 符合预期（✅ 通过）
- **执行步骤**：
  1. PM 根据 PRD 创建任务
  2. 任务拆解为 Epic/Feature 级别
  3. 锁定本迭代范围
  4. 通知开发团队开始工作
- **结果**：流程成功结束，进入开发阶段

#### 分支 B：审批不通过路径
- **触发条件**：用户认为 PRD 存在偏差或遗漏（❌ 不通过）
- **执行步骤**：
  1. 用户提出具体修改意见
  2. PM 记录反馈并更新 PRD 版本
  3. 返回需求分析阶段
- **结果**：进入迭代循环，重新分析和生成 PRD
